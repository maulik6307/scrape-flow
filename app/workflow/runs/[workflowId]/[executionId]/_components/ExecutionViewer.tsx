                        ...
                        <div className='flex flex-col py-4 container gap-4 overflow-auto'>
                            <div className='flex gap-2 items-center'>
                                ...
                                <Badge variant={"outline"} className='space-x-4'>
                                    ...
                                    <span>{DatesToDurationString(phaseDetails.data.completeAt, phaseDetails.data.startedAt) || "-"}</span>
                                </Badge>
                            </div>

                            <ParamaterViewer title="Inputs" subTitle="Inputs used for this phase" paramsJSON={phaseDetails.data.inputs} />
                            <ParamaterViewer title="Outputs" subTitle="Outputs geenrated by this phase" paramsJSON={phaseDetails.data.outputs} />
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    }
                    {
                        params && Object.entries(params).map(([key, value]) => (
                            <div key={key} className='flex justify-between items-center space-y-1'><p className='text-sm text-muted-foreground flex-1 basis-1/3'>{key}</p>
                ...
                <TableBody>
                    {
                        logs.map(log => (
                            <TableRow className='text-muted-foreground ' key={log.id}>
                                <TableCell width={190} className='text-xs text-muted-foreground p-[2px] pl-4'>{log.timestamp.toLocaleString()}</TableCell>
