...
const statusColors = {
    [WorkflowStatus.DRAFT]: "bg-yellow-500 text-white",
            ...
            <CardContent className='p-4 flex items-center justify-between h-[100px]'>
                <div className='flex items-center justify-end space-x-3'>
                    <div className={cn("w-10 h-10 rounded-full flex items-center justify-center", statusColors[workflow.status as WorkflowStatus])}>
                        {
                            isDraft ? <FileTextIcon className='w-5 h-5' /> : <PlayIcon className='w-5 h-5' color='#fff' />
                        }
                    </div>
                    <div>
...
function SchedulerSection({ isDraft, creditsCost, workflowId, cron }: { isDraft: boolean, creditsCost: number, workflowId: string, cron: string | null }) {
...
function LastRunDetails({ workflow }: { workflow: Workflow }) {
    ...
    return <div className='bg-primary/5 px-4 py-1 flex justify-between items-center text-muted-foreground'>
        <div className='flex items-center text-sm gap-2'>
            {lastRunAt && <Link href={`/workflow/runs/${workflow.id}/${workflow.lastRunId}`} className='flex items-center text-sm gap-2 group'>
                <span>
                    Last run:
                </span>
                ...
                <ChevronRightIcon size={14} className='-translate-x-[2px] group-hover:translate-x-0 transition-all' />
            </Link>}
            {
                !lastRunAt && (
                    <span>
